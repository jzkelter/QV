...
to spawn-voters
  (ifelse
    ...
    utility-distribution = "Prop8-normal mean>0" [create-voters-with-prop8-normal-utility-distributions]
    utility-distribution = "Prop8 mean>0 all issues" [create-voters-with-all-prop8-utility-dist]
    [  ; default option
  ...
  ask voters [ set-perceived-utilities ]
end
...
to create-voters-with-prop8-normal-utility-distributions
  ; This calibration is based off of sections 2.1 and 2.4 in Chandar and Weyl 2019, but adpated to use normal distrubtions
  ; utility units here are in units of $10k
  ...
  create-voters number-of-voters [
    ...
    (ifelse
      random-num < 0.007 [ ; This is the portion of the population that is LGBT and in a relationship
        ...
        set utilities fput (random-normal 0.5 0.2) utilities  ; same mean as previous line, but using normal distribution
      ]
      random-num < 0.04 [ ; This the portion of the population that is LGBT and in a relationship
        ; set utilities fput (0.025 + random-float .175) utilities
        set utilities fput (random-normal 0.15 0.05) utilities  ; same mean as previous line, but using normal distribution
      ]
      random-num < .48 [ ; This is the % of the non-LGBT population that supported gay marriage
        ; set utilities fput (random-float .1) utilities
        set utilities fput (random-normal 0.05 0.025) utilities  ; same mean as previous line, but using normal distribution
      ]
      [ ; the remaining % of the population opposed gay marriage
        ; set utilities fput (random-float -.1) utilities
        set utilities fput (random-normal -0.05 0.025) utilities  ; same mean as previous line, but using normal distribution
...
to create-voters-with-all-prop8-utility-dist
  ; This calibration is based off of sections 2.1 and 2.4 in Chandar and Weyl 2019
  ; utility units here are in units of $10k
...
to set-utilities
  ...
  if issues-correlation != 0 [
    ...
    repeat number-of-issues - correlate-n-issues [
      set temp-utilities fput (item curr-issue utilities) temp-utilities
      set curr-issue curr-issue + 1
    ]
    set utilities temp-utilities
  ]
end

to set-perceived-utilities
  set perceived-utilities map [i -> random-normal i perceived-utility-stdev] utilities
end
...
;  ]
  ...
  set votes (map [[e-o u] -> .5 * psi-prime(e-o) * u] estimated-outcome perceived-utilities)
